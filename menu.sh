...
show_menu() {
	...
	while true; do
		...
		1)
			...
			while true; do
				...
				1)
					...
					;;
				a)
					opti1=$(whiptail --title "${smenu[sipfunifacetitle]}" --inputbox "${smenu[sipfunifaceinput]}" --ok-button "${smenu[ok]}" --cancel-button "${smenu[cancel]}" 10 60 "$opti1" 3>&1 1>&2 2>&3)
					continue
					;;
				b)
					opti2=$(whiptail --title "${smenu[sipfunproxytitle]}" --inputbox "${smenu[sipfunproxyinput]}" --ok-button "${smenu[ok]}" --cancel-button "${smenu[cancel]}" 10 60 "$opti2" 3>&1 1>&2 2>&3)
					continue
					;;
				c)
					if
		...
		2)
			...
			while true; do
				...
				2)
					...
					;;
				3)
					optn1=$(whiptail --title "${smenu[snetroutetitle]}" --inputbox "${smenu[snetrouteinput]}" --ok-button "${smenu[ok]}" --cancel-button "${smenu[exit]}" 10 60 3>&1 1>&2 2>&3)
					cmd+=" -R $optn1"
					[[ $optn3 -eq 1 ]] && cmd+=" -y"
					[[ $optn4 -eq 1 ]] && cmd+=" -f"
					break
				...
				4)
					...
					;;
				5)
					optn2=$(whiptail --title "${smenu[snetcustomtitle]}" --checklist "${smenu[snetcustominput]}" --ok-button "${smenu[ok]}" --cancel-button "${smenu[exit]}" 16 60 7 \
		...
		3)
			...
			;;
		4)
			cmd="bash <(curl -sL https://Media.Check.Place)"
			;;
		5)
			cmd="bash <(curl -sL https://run.NodeQuality.com)"
			;;
		*) exit 1 ;;
		esac
